...
from autodet import jsondet, xmldet, yolodet
...
from libs.create_ml_io_custom import CreateMLReader
from libs.create_ml_io_custom import JSON_EXT
...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, default_filename=None, default_prefdef_class_file=None, default_save_dir=None):
        ...
        detect = action(get_str('openDir'), self.detect,
                                 'Ctrl+d', 'detect', get_str('openDir'))
        ...
        self.actions = Struct(save=save, save_format=save_format, saveAs=save_as, open=open, close=close, resetAll=reset_all, deleteImg=delete_image,
                              ...
                              fileMenuActions=(
                                  open, open_dir, save, save_as, close, detect, reset_all, quit),
                              beginner=(), advanced=(),
        ...
        add_actions(self.menus.file,
                    (open, open_dir, change_save_dir, open_annotation, copy_prev_bounding, detect, self.menus.recentFiles, save, save_format, save_as, close, reset_all, delete_image, quit))
        ...
        self.actions.beginner = (
            open, open_dir, change_save_dir, open_next_image, open_prev_image, detect, verify, save, save_format, None, create, copy, delete, None,
            zoom_in, zoom, zoom_out, fit_window, fit_width)

        self.actions.advanced = (
            open, open_dir, change_save_dir, open_next_image, open_prev_image, detect, save, save_format, None,
    ...
    def open_annotation_dialog(self, _value=False):
        ...
        if self.label_file_format == LabelFileFormat.CREATE_ML:
    ...
    def import_dir_images(self, dir_path):
        ...
        self.img_count = len(self.m_img_list)
        # 이미지마다 경로를 불러올 때 annotation 파일을 같이 불러 옴
        for imgPath in self.m_img_list:
            p = PurePath(imgPath)
            imgtype = p.parts[-1].split('.')[1]
            p = str(p)
            if self.label_file_format == LabelFileFormat.PASCAL_VOC:
                anno_Path = p.replace(imgtype, 'xml')
    ...
    def detect(self, _value=False, dir_path=None, silent=False):
        if not self.may_continue():
            return
        # 이미지가 담긴 폴더 경로 호출
        default_open_dir_path = dir_path if dir_path else '.'
        ...
        modelfilters = 'model file (onnx)(*%s)' % '*.onnx'
        # 모델 경로 호출
        default_model_path = dir_path if dir_path else '.'
        if self.last_open_dir and os.path.exists(self.last_open_dir):
            default_model_path = self.last_open_dir
        else:
            default_model_path = os.path.dirname(self.file_path) if self.file_path else '.'
        if silent != True:
            target_model_path = ustr(QFileDialog.getOpenFileName(self,
                                                                    '%s - Open model file(onnx, pt)' % __appname__, default_model_path,
                                                                    modelfilters))
        else:
            target_model_path = ustr(default_model_path)
        self.model_path = target_model_path[0]

        classfilters = 'class file (txt)(*%s)' % '*.txt'
        # 클래스 정보가 담긴 파일 경로 호출
        default_class_path = dir_path if dir_path else '.'
        if self.last_open_dir and os.path.exists(self.last_open_dir):
            default_class_path = self.last_open_dir
        else:
            default_class_path = os.path.dirname(self.file_path) if self.file_path else '.'
        if silent != True:
            target_class_path = ustr(QFileDialog.getOpenFileName(self,
                                                                    '%s - Open class file(txt)' % __appname__, default_class_path,
                                                                    classfilters))
        else:
            target_class_path = ustr(default_class_path)
        self.class_path = target_class_path[0]
        
        #auto detect 후 파일 생성 및 이미지와 같은 경로에 저장
        if self.label_file_format == LabelFileFormat.PASCAL_VOC:
            xmldet(target_dir_path+'/*', self.model_path, self.class_path)        

        elif self.label_file_format == LabelFileFormat.YOLO:
            yolodet(target_dir_path+'/*', self.model_path, self.class_path)       

        elif self.label_file_format == LabelFileFormat.CREATE_ML:
            jsondet(target_dir_path+'/*', self.model_path, self.class_path)
        self.file_path = target_dir_path
    ...
    def load_create_ml_json_by_filename(self, json_path):
        if self.file_path is None:
            return
        ...
        create_ml_parse_reader = CreateMLReader(json_path)
